...
class StellarEvolutionModel:
    ...
    def plot_evolution(self, evols, show=True):
        ...
        fig, axs = plt.subplots(1, 1, figsize=[10,14], sharex=True)

        for evol in evols:
            lxray = evol["final.star.LXRAY"].to(self.Lxray_unit)

            axs[0].plot(evol["Time"], lxray, color="k", alpha=0.5)

        axs[0].set_ylabel("Xray Luminosity [{}]".format(lxray.unit), fontsize=20)
        axs[0].set_xlabel("Time [{}]".format(evol["Time"].unit), fontsize=20)
        axs[0].set_xscale('log')
        axs[0].set_yscale('log')

        if self.Lxray_data is not None:
            axs[0].axhline(self.Lxray_data[0], color="r", linestyle="--")
            axs[0].axhspan(self.Lxray_data[0]-self.Lxray_data[1], self.Lxray_data[0]+self.Lxray_data[1], color="r", alpha=0.2)
    ...
    def plot_evolution_multi(self, evols, show=True):
        ...
        axs[1].set_xlabel("Time [{}]".format(evol["Time"].unit), fontsize=20)
        axs[0].set_xscale('log')
        axs[0].set_yscale('log')

        axs[1].set_ylabel("Xray Luminosity [{}]".format(lxray.unit), fontsize=20)
        axs[1].set_xlabel("Time [{}]".format(evol["Time"].unit), fontsize=20)
...
if __name__ == '__main__':
    ...
    model = StellarEvolutionModel(star_name="HD 219134",
                                  Lbol_data=Lbol_data, 
                                  Lxuv_data=Lxuv_data, 
                                  #Prot_data=Prot_data,
                                  age_data=age_data)
